...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        if self.volume < len(self.contents):
            self.contents = ''
        self.contentsstart = self.contents
        self.volumestart = self.volume
        self.h = []
    ...
    def double_then_shrink(self, new_volume):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # ---------------------------------------------------------------------
        x = 2 * len(self.contents)
        self.double()
        self.shrink(new_volume)
        return x - len(self.contents)
    ...
    def reset(self):
        ...
        self.contents = self.contentsstart
        self.volume = self.volumestart
